export async function getRecipes() {
    ...
    const ingredients = recipes.map(recipe => recipe.ingredients.map(name => name.ingredient.toLowerCase())).flat()
    const appareils = recipes.map(recipe => recipe.appliance.toLowerCase())
    const ustensils = recipes.map(recipe => recipe.ustensils.map(name => name.toLowerCase())).flat()
